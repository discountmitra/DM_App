...
type HomeServiceCategoryKey = string;
type HomeService = {
  id: string;
  name: string;
  description: string;
...
export default function HomeServicesScreen() {
  ...
  const categories = useMemo(() => ["All", ...Array.from(new Set(data.map(s => s.category)))], [data]);

  useEffect(() => {
    const pre = params.preselect as string | undefined;
    if (pre && categories.includes(pre)) {
      setSelectedCategory(pre as HomeServiceCategoryKey);
      return;
    }
    if (!pre && categories.length && !categories.includes(selectedCategory)) {
      setSelectedCategory(categories[0] as HomeServiceCategoryKey);
    }
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        setError(null);
        const res = await fetch(`${BASE_URL}/home-services`);
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const json = await res.json();
        if (alive) setData(json as HomeService[]);
      } catch (e: any) {
